...
export const useCountIsto = defineStore("countIsto", {
  getters: {
    ...
    biggestEntry() {
      ...
      return istoStore.istos.reduce(
        (max, isto) => ((parseFloat(isto.total) || 0) > (parseFloat(max.total) || 0) ? isto : max),
        istoStore.istos[0]
      )
    },
    ...
    smallestEntry() {
      ...
      if (!istoStore.istos.length) return { title: "-", total: 0 }
      return istoStore.istos.reduce(
        (min, isto) => ((parseFloat(isto.total) || 0) < (parseFloat(min.total) || 0) ? isto : min),
        istoStore.istos[0]
      )
    }
  }
