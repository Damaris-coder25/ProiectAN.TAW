...
<template>
  <div ref="editContainer" class="grid grid-cols-6 gap-2" @click.stop>
    <div v-if="!isEditMode" class="col-span-3" @click="isEditMode = true">
      {{ new Date(isto.date).toLocaleDateString() }}<br />{{ isto.title }} - {{ isto.total }}
    </div>
    <div v-else class="col-span-3">
      <input
        v-model="istoEditTitle"
        type="text"
        class="w-full bg-gray-100"
        @keyup.enter="saveIstoTitle"
      />
      <input
        v-model="istoEditTotal"
        type="number"
        class="w-full bg-gray-100"
        @keyup.enter="saveIstoTotal"
      />
      <input
        v-model="istoEditDate"
        type="date"
        class="w-full bg-gray-100"
        @keyup.enter="saveIstoDate"
      />
    </div>
